...
func NewStudentServer(repo repository.Repository) *Server {
	return &Server{
		repo:                              repo,
		UnimplementedStudentServiceServer: studentpb.UnimplementedStudentServiceServer{},
	}
}

// Referencia a nuestro archiv proto

func (s *Server) GetStudent(ctx context.Context, req *studentpb.GetStudentRequest) (*studentpb.Student, error) {

	student, err := s.repo.GetStudent(ctx, req.GetStudentId())
	if err != nil {
		return nil, err
	}
